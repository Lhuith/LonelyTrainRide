...
SubShader
	{
		...
		v2f vert (appdata_base v)
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				i.lightDir, specularReflection * fresnelFactor, i.viewDir) * fresnelFactor;
		
				glitterSpec *= _GlitterStrength * 12;

			    return float4(float4(glitterSpec,1.0)) ; 
		...
		v2f vert (appdata_base v)
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 waterColor = GetWaterColor(fadeDist / 25, waterDepth, refraction, diffuseReflection + specularReflection);		
			    return float4((waterColor + (reflection * fresnelFactor)) + lightFinal, fade);
